...
public class ServerGroup {
         ...
         * @return group info
         */
        public static ServerGroup loadFromFile() throws Exception {
                ServerGroup serverGroup;
                ...
                serverGroup = new ServerGroup();
                keep_alive = SystemConfig.getInt("fdht.keep_alive", 0);
                serverGroup.keep_alive = keep_alive != 0;

                group_count =  SystemConfig.getInt("fdht.group_count", 0);
                if (group_count <= 0) {
                ...
                for (int i = 0; i < group_count; i++) {
                        ...
                        szServers =  SystemConfig.getStringArray("fdht.group" + i);
                        
                        if (szServers == null || szServers.length == 0) {
                                throw new Exception("item \"group" + i + "\" in configFile"
                                                + " not found");
                        }
