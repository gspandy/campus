...
public class FastDHTClient {
    private static Logger logger = LoggerFactory.getLogger(FastDHTClient.class);
    protected ServerGroup serverGroup;
    protected byte status;

    public FastDHTClient() throws Exception {
        this(true);
    }
     ...
     * @throws Exception 
     */
    public FastDHTClient(boolean bMultiThread) throws Exception {
        try {
            ClientGlobal.init();
        } catch (Exception e) {
    ...
    private int parseBatchUpdate(int src_key_count, ServerInfo server, ProtoCommon.PkgInfo pkgInfo, Map failKeys)
        ...
        if (key_count != src_key_count) {
            System.err.println("server " + server.address.getAddress().getHostAddress() + ":"
                    + server.address.getPort() + " key count: " + key_count + " != " + src_key_count);
