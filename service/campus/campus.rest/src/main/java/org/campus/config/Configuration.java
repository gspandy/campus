...
public interface Configuration {
     ...
     * a typical implementation of the {@code Configuration} interface the other get methods (that return specific data
     * types) will internally make use of this method. On this level variable substitution is not yet performed. The
    ...
    /**
     * Get the set of the keys contained in the configuration that match the specified prefix. For instance, if the
     ...
     * Note that the prefix itself is included in the result set if there is a matching key. The exact behavior - how
     * the prefix is actually interpreted - depends on a concrete implementation.
     *
    ...
    /**
     * Get the set of the keys contained in the configuration. The returned iterator can be used to obtain all defined
     * keys. Note that the exact behavior of the iterator's {@code remove()} method is specific to a concrete
     * implementation. It <em>may</em> remove the corresponding property from the configuration, but this is not
     * guaranteed. In any case it is no replacement for calling clearProperty for this property. So it is highly
    ...
    /**
     * Get a list of properties associated with the given configuration key. This method expects the given key to have
     * an arbitrary number of String values, each of which is of the form {code key=value}. These strings are split at
     * the equals sign, and the key parts will become keys of the returned {@code Properties} object, the value parts
     * become values.
     * 
    ...
    /**
     * Get a float associated with the given configuration key. If the key doesn't map to an existing object, the
     * default value is returned.
     * 
     * @param key The configuration key.
     * @param defaultValue The default value.
    ...
    /**
     * Get a {@link Float} associated with the given configuration key. If the key doesn't map to an existing object,
    ...
    /**
     * Get a {@link Short} associated with the given configuration key. If the key doesn't map to an existing object,
     * the default value is returned.
     * 
     * @param key The configuration key.
     * @param defaultValue The default value.
